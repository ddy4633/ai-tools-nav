...
export default async function ChatbotCategoryPage() {
  ...
  const tools = allTools.filter((tool: Tool) => 
    tool.category?.includes('聊天') || 
    tool.category?.toLowerCase().includes('chat') ||
    tool.category?.includes('对话') ||
    tool.category?.includes('助手')
  );
    ...
    <div className="min-h-screen bg-bg-primary">
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <nav className="flex items-center gap-2 text-sm text-text-muted mb-8">
          ...
          <span>/</span>
          <span className="text-text-primary">聊天机器人</span>
        </nav>
        ...
        <div className="mb-8">
          ...
          <p className="text-text-secondary max-w-2xl">
            共 {tools.length} 个AI聊天机器人工具，包括AI对话、智能助手、AI客服等。
            这些工具可以提供智能对话服务，帮助解答问题和完成任务。
          </p>
        </div>
        ...
        {tools.length > 0 ? (
          ...
          <div className="text-center py-16 bg-white rounded-xl shadow-soft">
            ...
            <p className="text-text-primary text-lg mb-2">暂无聊天机器人数据</p>
