...
export const metadata: Metadata = {
  ...
  description: '发现最好的AI生产力工具，包括效率工具、任务管理、时间管理、办公助手等各类人工智能生产力提升工具。',
  keywords: ['AI生产力', '效率工具', '任务管理', '时间管理', '人工智能办公'],
};
...
export default async function ProductivityCategoryPage() {
  ...
  const tools = allTools.filter((tool: Tool) => 
    tool.category?.includes('生产力') || 
    tool.category?.includes('效率') ||
    tool.category?.toLowerCase().includes('productivity') ||
    tool.category?.includes('任务') ||
    tool.category?.includes('办公')
  );
    ...
    <div className="min-h-screen bg-bg-primary">
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <nav className="flex items-center gap-2 text-sm text-text-muted mb-8">
          ...
          <span>/</span>
          <span className="text-text-primary">生产力</span>
        </nav>
        ...
        <div className="mb-8">
          ...
          <p className="text-text-secondary max-w-2xl">
            共 {tools.length} 个AI生产力工具，包括效率工具、任务管理、时间管理、办公助手等。
            这些工具可以帮助你提升工作效率，更好地管理时间和任务。
          </p>
        </div>
        ...
        {tools.length > 0 ? (
          ...
          <div className="text-center py-16 bg-white rounded-xl shadow-soft">
            ...
            <p className="text-text-primary text-lg mb-2">暂无生产力工具数据</p>
