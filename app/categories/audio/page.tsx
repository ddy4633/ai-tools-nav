...
export default async function AudioCategoryPage() {
  ...
  const tools = allTools.filter((tool: Tool) => 
    tool.category?.includes('音频') || 
    tool.category?.toLowerCase().includes('audio') ||
    tool.category?.includes('音乐') ||
    tool.category?.includes('语音') ||
    tool.category?.includes('配音')
  );
    ...
    <div className="min-h-screen bg-bg-primary">
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <nav className="flex items-center gap-2 text-sm text-text-muted mb-8">
          ...
          <span>/</span>
          <span className="text-text-primary">音频</span>
        </nav>
        ...
        <div className="mb-8">
          ...
          <p className="text-text-secondary max-w-2xl">
            共 {tools.length} 个AI音频工具，包括AI音乐生成、语音合成、音频编辑、AI配音等。
            这些工具可以帮助你创作音乐、生成语音、编辑音频内容。
          </p>
        </div>
        ...
        {tools.length > 0 ? (
          ...
          <div className="text-center py-16 bg-white rounded-xl shadow-soft">
            ...
            <p className="text-text-primary text-lg mb-2">暂无音频工具数据</p>
