...
export default async function WritingCategoryPage() {
  ...
  const tools = allTools.filter((tool: Tool) => 
    tool.category?.includes('写作') || 
    tool.category?.toLowerCase().includes('writing') ||
    tool.category?.includes('文案') ||
    tool.category?.includes('内容')
  );
    ...
    <div className="min-h-screen bg-bg-primary">
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <nav className="flex items-center gap-2 text-sm text-text-muted mb-8">
          ...
          <span>/</span>
          <span className="text-text-primary">写作</span>
        </nav>
        ...
        <div className="mb-8">
          ...
          <p className="text-text-secondary max-w-2xl">
            共 {tools.length} 个AI写作工具，包括文案生成、内容创作、博客写作、营销文案等。
            这些工具可以帮助你快速生成高质量的文字内容，提升写作效率。
          </p>
        </div>
        ...
        {tools.length > 0 ? (
          ...
          <div className="text-center py-16 bg-white rounded-xl shadow-soft">
            ...
            <p className="text-text-primary text-lg mb-2">暂无写作工具数据</p>
